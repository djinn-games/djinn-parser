...
const BLOCKS =  {
    ...
    'RepeatUntilSentence': translateRepeatUntil,
    'FromSentence': translateFrom
};
...
function translateRepeatUntil(scope, block) {
    let subscope = scope.push();
    subscope.flagInLoop();

    Asserters.assertDataType(block.test, ['bool']);
    ...
    return {
        ...
        test: { // it's repeat-until and not do-while, so negate the test
            type: 'UnaryExpression',
            operator: '!',
            argument: translateBlock(subscope, block.test),
            prefix: true
        }
    };
}

function translateFrom(scope, block) {
    // TODO: look up how to clone objects

    let subscope = scope.push();
    subscope.flagInLoop();

    // assert data types
    Asserters.assertDataType(block.init, ['int']);
    Asserters.assertDataType(block.to, ['int']);
    Asserters.assertDataType(block.step, ['int']);

    let counterId = block.init.id || block.init.left;
    ...
    return {
        type: 'ForStatement',
        init: translateBlock(subscope, block.init),
        test: translateBlock(subscope, testBlock),
        update: translateBlock(subscope, updateBlock),
        body: {
            type: 'BlockStatement',
            body: translateBody(subscope, block)
        }
...
function translateElseList(scope, blockList) {
    ...
    // TODO: dont open a new scope for every elseif
    scope = scope.pop()
