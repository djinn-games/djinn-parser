...
function translateIf(scope, block) {
    let subscope = scope.addSubScope();

    let condition = translateBlock(subscope, block.if.condition);
    Asserters.assertDataType(block.if.condition, ['bool']);

    let alternates = block.if.alternates;
    if (alternates.length > 0 && alternates[0].length > 0 && alternates[0][0].type === 'IfSentence') {
        console.log("???");
        alternates[0].if.alternates = alternates.slice(1, alternates.length);
        console.log(alternates[0]);
        alternates = alternates.slice(0, 1); // get only first element
        console.log(alternates);
    }
    else if (alternates.length > 0){
        console.log('ARRAY', alternates[0][0]);
    }
    else {
        console.log('NONE',alternates);
    }
    ...
    return {
       ...
       alternate: {
           type: 'BlockStatement',
           body: alternates.length > 0
                 ? alternates[0].map(function (x) {
                     return translateBlock(subscope, x);
                 })
                 : []
       }
