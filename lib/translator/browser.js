...
Translator.prototype._translateIdentifier = function (scope, block, isFunction) {
    ...
    if (isFunction) {
        ...
        return {
            ...
            property: this._idToLiteral(lookup),
            computed: true
        };
    }
};
...
Translator.prototype._translateVarDeclaration = function (scope, block) {
    ...
    block.id.name = scope.getVar(block.id.name).name;

    // no initialisation? we need to manually include a default one
    let init = block.init
        ? this._translateBlock(scope, block.init)
        : this._defaultLiteralForType(block.dataType);
    // assert data type of initialization, unless it was one by default
    if (block.init) { this._assertDataType(block.init, [block.dataType]); }

    // TODO: refactor this in a assignment helper
    return this._buildAssignment(block.id, init, '=');
};

Translator.prototype._translateAssignment = function (scope, block) {
    // check that variable exists in scope
    this._assertIdentifierExists(scope, block.left);

    // TODO: check this is not a constant

    // check data type
    let right = this._translateBlock(scope, block.right);
    let lookup = scope.getVar(block.left.name);
    this._assertDataType(block.right, [lookup.dataType]);

    block.dataType = lookup.dataType;

    return this._buildAssignment(lookup, right, block.operator);
};
...
Translator.prototype._buildAssignment = function (name, right, operator) {
    return {
        ...
        left: {
            ...
            property: this._idToLiteral(name),
            computed: true
        },
        right: right
    };
