...
describe('Translator asserts', function () {
    ...
    describe('Identifier existence assert', function () {
        beforeEach(function () {
            this.revert = Translator.__set__('FUNC_DEFS', {
                vader: { args: [['str']] }
            });
        ...
        it('throws an error when the id doesn\'t exist', sinon.test(function () {
            sinon.stub(this.scope, 'getVarOrConst').returns(null);
            expect(function () {
                this.translator._assertIdentifierExists(
        ...
        it('passes when the id is a existing variable', sinon.test(function () {
            let stub = sinon.stub(this.scope, 'getVarOrConst').returns(null);
    ...
    describe('Function arguments assert', function () {
        ...
        it('throws an error when the number of args is wrong', function () {
            expect(function () {
                this.translator._assertFunctionArgs(buildCall('vader', []));
        ...
        it('checks for the type of args', sinon.test(function () {
            sinon.stub(this.translator, '_assertDataType');

            this.translator._assertFunctionArgs(buildCall('vader', ['int']));

            expect(this.translator._assertDataType.calledOnce).to.be.true;
            expect(this.translator._assertDataType.calledWith(
                sinon.match.any, ['str'])).to.be.true;
        }));

        it('passes when the number and type of args is correct', function () {
            expect(function () {
                this.translator._assertFunctionArgs(buildCall('vader', ['str']));
            }.bind(this)).to.not.throw(Error);
        });
    });
