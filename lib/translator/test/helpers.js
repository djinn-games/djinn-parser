...
describe('Translator helpers', function () {
    ...
    it('check for div by zero for divs and mods', function () {
        expect(this.translator.shouldCheckForDivByZero({operator: 'div'}))
            .to.be.true;
        expect(this.translator.shouldCheckForDivByZero({operator: 'mod'}))
            .to.be.true;
        ['add', 'sub', 'mul', 'minus', 'plus'].forEach(function (op) {
            expect(this.translator.shouldCheckForDivByZero({operator: op}))
                .to.be.false;
        }.bind(this));
    });
    ...
    describe('Binary operations data type', function () {
        ...
        it('returns str for string concatenation', function () {
            // str + str
            expect(this.translator.getDataTypeForBinaryOperation(
    ...
    it('transform an ID to a literal', function () {
        ...
        expect(this.translator.idToLiteral(id)).to.deep.equal({
            type: 'Literal',
            value: 'waka',
            raw: '\"waka\"'
        });
    ...
    it('gets a default literal for a data type', function () {
        expect(this.translator.defaultLiteralForType('int')).to.deep.equal({
            type: 'Literal',
            value: 0,
            raw: '0'
        });
        expect(this.translator.defaultLiteralForType('float')).to.deep.equal({
            type: 'Literal',
            value: 0,
            raw: '0'
        });
        expect(this.translator.defaultLiteralForType('str')).to.deep.equal({
            type: 'Literal',
            value: '',
            raw: '\"\"'
        });
        expect(this.translator.defaultLiteralForType('bool')).to.deep.equal({
            type: 'Literal',
            value: false,
            raw: 'false'
        });
