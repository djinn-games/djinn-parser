...
describe('Translator helpers', function () {
    ...
    it('gets a default literal for a data type', function () {
        expect(this.translator._defaultLiteralForType('int')).to.deep.equal({
            type: 'Literal',
            value: 0,
            raw: '0'
        });
        expect(this.translator._defaultLiteralForType('float')).to.deep.equal({
            type: 'Literal',
            value: 0,
            raw: '0'
        });
        expect(this.translator._defaultLiteralForType('str')).to.deep.equal({
            type: 'Literal',
            value: '',
            raw: '\"\"'
        });
        expect(this.translator._defaultLiteralForType('bool')).to.deep.equal({
            type: 'Literal',
            value: false,
            raw: 'false'
        });
